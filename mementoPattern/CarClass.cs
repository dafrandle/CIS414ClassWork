    ...
    internal class CarClass
    {
        ...
        public Dictionary<string, string> ToDictionary()
        {
            Dictionary<string, string> carDictionary = new()
            {
                ...
                { "Doors", Doors },
                { "EnergyCapacity", EnergyCapacity.ToString() },
                { "EnergyType", EnergyType },
                { "MilesPerEnergyUnit", MilesPerEnergyUnit.ToString() },
                { "AccelerationMSsquared", AccelerationMSsquared.ToString() },
                { "WeightInKG", WeightInKG.ToString() },
                { "GVWRInKG", GVWRInKG.ToString() }
            };
            return carDictionary;
        }

        public void LoadFromDictionary(Dictionary<string, string> carDictionary)
        {
            Name = carDictionary["Name"];
            Size = carDictionary["Size"];
            Engine = carDictionary["Engine"];
            CenterConsole = carDictionary["CenterConsole"];
            CruiseControl = carDictionary["CruiseControl"];
            Doors = carDictionary["Doors"];
            EnergyCapacity = Convert.ToDouble(carDictionary["EnergyCapacity"]);
            EnergyType = carDictionary["EnergyType"];
            MilesPerEnergyUnit = Convert.ToDouble(carDictionary["MilesPerEnergyUnit"]);
            AccelerationMSsquared = Convert.ToDouble(carDictionary["AccelerationMSsquared"]);
            WeightInKG = Convert.ToDouble(carDictionary["WeightInKG"]);
            GVWRInKG = Convert.ToDouble(carDictionary["GVWRInKG"]);

            // if this was production code I would add error handling for failed casts to double.
        }
    }
}
